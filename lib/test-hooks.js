...
const Attachment = require('../model/attachment.js');
const Pic = require('../model/profile-pic.js');
...
hooks.createOrg = function (done) {
  ...
  new Org(this.exampleOrg).save()
    .then( org => {
      this.tempOrg = org;
      this.tempOrg.admins = [this.tempUser._id];
      return Org.findByIdAndUpdate(this.tempOrg._id, this.tempOrg, {new: true});
    })
    .then( org => {
      this.tempOrg = org;
      done();
...
hooks.createTask = function (done) {
  ...
  task.save()
    .then(task => {
      ...
      return Project.findByIdAndUpdate(this.tempProject._id, this.tempProject, {new: true});
    })
    .then( project => {
...
hooks.createProject = function (done) {
  ...
  new Project(this.exampleProject).save()
    .then(project => {
      this.tempProject = project;
      this.tempOrg.projects = [this.tempProject._id.toString()];
      return Org.findByIdAndUpdate(this.tempOrg._id, this.tempOrg, {new: true});
    })
    .then( org => {
      this.tempOrg = org;
